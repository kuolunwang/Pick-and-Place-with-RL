...
#include <vx300s_bringup/ee_pose.h>
...
class calibration{
 private:
  ...
  vx300s_bringup::ee_pose req; // request for server
  ...
  bool get_arm_data(tf::StampedTransform &t, std::string target){
    try{
      listener.waitForTransform("vx300s/base_link", target, ros::Time(0), ros::Duration(2.0));
      listener.lookupTransform("vx300s/base_link", target, ros::Time(0), t);
    } catch(tf::TransformException ex){
      ROS_ERROR("%s", ex.what());
      return 0;
    }
 ...
 public:
  ...
  calibration(ros::NodeHandle nh, ros::NodeHandle pnh): nh_(nh), pnh_(pnh), count(0){
    ...
    ros::service::waitForService("/vx300s/go_pose"); ROS_INFO("Service connect!");
    client = nh_.serviceClient<vx300s_bringup::ee_pose>("/vx300s/go_pose", true);
